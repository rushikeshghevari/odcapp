...
export default function CourierEntries() {
    ...
    async function loadInitialData() {
        ...
        // Automatically pre-fill the user's own phone number to ensure they own the entry
        if (!phoneNumber) {
    ...
    async function openCamera(entryId: number) {
        try {
            ...
            Alert.alert(
                "Action Failed",
                error?.response?.data?.message || error?.message || "Could not launch camera or upload the photo. Please try again."
            );
        }
    }
    ...
    async function handleSubmit() {
        ...
        if (!result.success) {
            const firstError =
                result.error.issues[0]?.message || "Invalid courier entry data";
        ...
        <SafeAreaView className="flex-1 bg-gray-100" edges={["top"]}>
            ...
            >
                ...
                <View className="rounded-2xl bg-white p-4 shadow-md">
                    ...
                    <TextInput
                        className="mb-3 rounded-xl border border-gray-300 bg-white px-4 py-3 text-sm text-gray-900"
                    ...
                    <TextInput
                        className="mb-4 rounded-xl border border-gray-300 bg-white px-4 py-3 text-sm text-gray-900"
                ...
                {/* Recent Entries - only this area scrolls */}
                        ...
                        <View className="mb-5 items-center">
                            {selectedEntry?.photoUri ? (
                                ...
                                <View className="h-40 w-40 items-center justify-center rounded-2xl border border-dashed border-gray-300 bg-gray-50">
                                    <Ionicons name="image-outline" size={38} color="#9CA3AF" />
